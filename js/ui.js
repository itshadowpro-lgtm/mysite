...
function forceResizeMatrix() {
    ...
    // Only resize if matrix is still active (not stopped)
    if (matrixCanvas && !window.matrixStopped) {
...
document.addEventListener('fullscreenchange', forceResizeMatrix);
...
// 👉 show hint text
const hint = document.getElementById('gif-hint');
...
if (currentSettings.enableHeart === true) {
    setTimeout(() => {
        startHeartEffect();
           ...
           // 🛑 If sequence is finished, stop matrix with small delay
if (sequence.length === 0 && !action) {
    setTimeout(() => {
        stopWebsite();
        window.matrixStopped = true;
