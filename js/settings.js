...
let settings = {
    ...
    sequence: 'Loveeeeuhhhh|Sooo|Muchhhh|Jaaannnnn|💋',
...
const musicOptions = [
    ...
    { value: 'audio/Sawaal.mp3', label: 'Home page song' },
    { value: 'audio/happybirthday_domixi.mp3', label: 'song3 (not uploaded)' },
    { value: 'audio/happybirtday_uia.mp3', label: 'song4 (not uploaded)' },
    { value: 'audio/music.mp3', label: 'song5 (not uploaded)' },
    { value: 'audio/postcardnam.mp3', label: 'song6 (not uploaded)' },
    { value: 'audio/podcardnu.mp3', label: 'song7 (not uploaded)' },
    { value: 'audio/givemeyourforever.mp3', label: 'song8 (not uploaded)' },
    { value: 'audio/perfect.mp3', label: 'song9 (not uploaded)' },
    { value: 'audio/phepmau.mp3', label: 'song10 (not uploaded)' },
    { value: 'audio/lambantraianhe.mp3', label: 'song11 (not uploaded)' },
    { value: 'audio/denbenanh.mp3', label: 'song12 (not uploaded)' },
    { value: 'audio/anhnangcuaanh.mp3', label: 'song13 (not uploaded)' },
    { value: 'audio/dunglamtraitimanhdau.mp3', label: 'song14 (not uploaded)' },
    { value: 'audio/buiphan.mp3', label: 'song15 (not uploaded)' },
    { value: 'audio/buocnhevaotimanh.mp3', label: 'song16 (not uploaded)' },
    { value: 'audio/codoidieu.mp3', label: 'song17 (not uploaded)' },
    { value: 'audio/coem.mp3', label: 'song18 (not uploaded)' },
    { value: 'audio/emnay.mp3', label: 'song19 (not uploaded)' },
    { value: 'audio/emlacogiaovungcao.mp3', label: 'song20 (not uploaded)' },
    { value: 'audio/emlacogiaovungcaoremix.mp3', label: 'song21 (not uploaded)' },
    { value: 'audio/lathuguithay.mp3', label: 'song22 (not uploaded)' },
    { value: 'audio/loithayco.mp3', label: 'song23 (not uploaded)' },
    { value: 'audio/nguoithay.mp3', label: 'song24 (not uploaded)' },
    { value: 'audio/nguoigieomamxanh.mp3', label: 'song25 (not uploaded)' },
    { value: 'audio/nguoidautien.mp3', label: 'song26 (not uploaded)' },
    { value: 'audio/motdoi.mp3', label: 'song27 (not uploaded)' },
    { value: 'audio/minhcungnhaudongbang.mp3', label: 'song28 (not uploaded)' },
    { value: 'audio/neunhucohaitraitim.mp3', label: 'song29 (not uploaded)' },
    { value: 'audio/ngayxuaay.mp3', label: 'song30 (not uploaded)' },
    { value: 'audio/Như 1 vì tinh tú.mp3', label: 'song31 (not uploaded)' },
    { value: 'audio/tinhyeudieuky.mp3', label: 'song32 (not uploaded)' },
    { value: 'audio/trentinhbanduoitinhyeu.mp3', label: 'song33 (not uploaded)' },
    { value: 'audio/tungngayyeuem.mp3', label: 'song34 (not uploaded)' },
    { value: 'audio/yeuemratnhieu.mp3', label: 'song35 (not uploaded)' },
    { value: 'audio/suynghitronganh.mp3', label: 'song36 (not uploaded)' }
];
const gifOptions = [
    { value: '', label: 'None' },
...
function getIdlePreviewMessage() {
    ...
    return label ? `Selected: ${label}` : 'Chọn bài nhạc rồi nhấn "Listen to a Sample"';
}

function setMusicPreviewState({ message, isPlaying }) {
    if (musicPreviewButton) {
        musicPreviewButton.textContent = isPlaying ? '⏸ Pause and Listen Other Songs' : '▶ Listen to a Sample';
        if (isPlaying) {
...
function handleMusicPreview() {
    ...
    if (!musicSelect || !musicSelect.value) {
        setMusicPreviewState({
            message: 'Vui lòng chọn một bài nhạc trước khi Listen to a Sample',
            isPlaying: false
        });
        return;
    }
    ...
    musicPreviewAudio.play().then(() => {
        ...
        stopMusicPreview('Không phát được bản Listen to a Sample. Vui lòng thử lại.');
    });
}
...
function resetWebsiteState() {
    ...
    // also hide gif hint
const gifHint = document.getElementById('gif-hint');
if (gifHint) {
    gifHint.style.display = 'none';
}
...
function updatePricingFromModal() {
    if (window.pricingCalculator) {
        const currentModalSettings = {
            music: document.getElementById('backgroundMusic')?.value || 'audio/happybirtday_uia.mp3',
...
if (languageSwitchBtn) {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            window.settings = {
                music: serverSettings.music || 'audio/happybirtday_uia.mp3',
...
applySettingsButton.addEventListener('click', () => {
    ...
    settings.sequence = document.getElementById('sequenceText').value || 'HAPPY|BIRTHDAY|MY|CUTEE|LITTLE|SONIYA|❤';
